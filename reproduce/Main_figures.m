...
xlabel(ax1, '$\mathbf{Nitrogen \; fixation \; efficiency \; (g\,C\,g^{-1}\,N)}$', ...
       'FontSize', 16, ...
       'Interpreter', 'latex');
...
if ~isempty(ymax2) && isfinite(ymax2)
    ylim(ax2, [0, ymax2*1.2]);  % add a little headroom
...
xlabel(ax3, '$\mathbf{Nitrogen \; fixation \; efficiency \; (g\,C\,g^{-1}\,N)}$', ...
       'FontSize', 16, ...
       'Interpreter', 'latex');
...
if ~isempty(ymax4) && isfinite(ymax4)
    ylim(ax4, [0, ymax2*1.2]);  % add a little headroom
end
...
% Custom offsets (fraction of axis range)
x_offsets = [-0.0015, -0.0025, -0.0015, -0.0025];   % Different for each axis
y_offsets = [-0.0035, -0.0035, -0.01, -0.01];
...
for i = 1:length(axes_handles)
    ...
    xpos = xlims(1) + x_offsets(i) * range(xlims);
    ypos = ylims(2) + y_offsets(i) * range(ylims);
    
    text(ax, xpos, ypos, labels{i}, ...
        'FontWeight', 'bold', ...
        'FontSize', 16, ...
...
% exportgraphics(figure(1), '../figures/Figure2.png', 'Resolution', 1000);
...
unknownKindex=[1,2,3,7,10,11,15,19,26,30,31,33,34,35];
...
yline(10.5, 'k-', 'LineWidth', 0.8);  
text(3.5, 8.5, 'Glycolysis', ...
    'HorizontalAlignment', 'center', ...
    'VerticalAlignment', 'top', ...
    'FontSize', 16,'FontWeight','bold');

yline(24.5, 'k-', 'LineWidth', 0.8);
text(3.5, 18.5, 'PP Pathway', ...
    'HorizontalAlignment', 'center', ...
    'VerticalAlignment', 'top', ...
    'FontSize', 16,'FontWeight','bold');

yline(30.5, 'k-', 'LineWidth', 0.8);  % value 是 y 位置
text(3.5, 28.5, 'TCA cycle', ...
    'HorizontalAlignment', 'center', ...
    'VerticalAlignment', 'top', ...
    'FontSize', 16,'FontWeight','bold');

text(3, 31.5, 'Glyoxylate cycle', ...
    'HorizontalAlignment', 'center', ...
    'VerticalAlignment', 'top', ...
    'FontSize', 16,'FontWeight','bold');

ynames_vALTN = filtered_vALTNout_SA.Var1;
unknownKindex=[1,2,3,10,14,18,24,27,29,30,31];
...
% remove Vmax_TKT2 and Vmax_ACN2 in newgata.Vmax and use Vmax_TKT, Vmax_ACN
% to replace Vmax_TKT1&2 and VmaxACN1&2
newdataVmax_filtered=newdata.Vmax;
newdataVmax_filtered([19,25],:)=[]; % TKT2 and ACN2

alpha=0.5;
[prcc, prcc_sign,sign_indices,sign_param,sign_prccvalues] = PRCC_modified(newdataVmax_filtered', newdata.v', Enames, alpha);
...
[prcc2, prcc_sign2,sign_indices2,sign_param2,sign_prccvalues2] = PRCC_modified(newdataVmax_filtered', newdata.gCgN', Enames, alpha);
...
[prcc, prcc_sign,sign_indices,sign_param,sign_prccvalues] = PRCC_modified(newdataVmax_filtered', newdata_vALTNout', Enames, alpha);
...
both_ICL_range=Vmax1.ICL .* scaler;
both_GD_range=Vmax1.GD .* scaler;
both_ALTSS_range=Vmax1.ALTSS .* scaler;
both_SHMT_range=Vmax1.SHMT .* scaler;
both_PRAT_range=Vmax1.PRAT .* scaler;
...
enzNames  = {'PGI','PFK','FBP','GDH','PYK',...
    ...
    'LPD','ACN', 'SDH','FUMA','MALOut','SUCout','FUMout',...
    'ICL', 'GD', 'ALTSS', 'SHMT',...
    'PRAT','FGAMS','IMPCH','IMPDH'};
enzRanges = {both_PGI_range, both_PFK_range,both_FBP_range,both_GDH_range, both_PYK_range,...
    both_ZWF_range,both_PGL_range, both_TKT_range, both_TAL_range, both_PEPC_range,...
    both_LPD_range, both_ACN_range, both_SDH_range, both_FUMA_range, ...
    both_MALout_range, both_SUCout_range, both_FUMout_range,...
    both_ICL_range, both_GD_range, both_ALTSS_range, both_SHMT_range,...
    both_PRAT_range, both_FGAMS_range, both_IMPCH_range, both_IMPDH_range};
...
% name for gcgn
enzyme_all={
    'FBP- FUMA+', 'FBP- LPD-','FBP- MALout+','FBP- PEPC+','FBP- PGL-','FBP- PYK-','FBP- SDH+','FBP- TAL-','FBP- ZWF-','FBP- ICL+',...
    'PGI+ FBP+','PGI+ FUMA+','PGI+ ICL+','PGI+ LPD-','PGI+ MALout+','PGI+ PEPC+','PGI+ PFK+','PGI+ PGL-','PGI+ PYK-','PGI+ SDH+','PGI+ TAL-','PGI+ ZWF-',...
    'ZWF- FUMA+','ZWF- ICL+','ZWF- LPD-','ZWF- MALout+','ZWF- PEPC+','ZWF- PGL-','ZWF- SDH+','ZWF- TAL-',...
    'FUMA+ ICL+', 'FUMA+ MALout+',...
    'LPD- FUMA+','LPD- ICL+','LPD- MALout+','LPD- SDH+',...
    'PGL+ FUMA+','PGL+ ICL+','PGL+ LPD-','PGL+ MALout+','PGL+ PEPC+','PGL+ SDH+','PGL+ TAL-',...
    'MALout+ ICL+',...
    'PEPC+ FUMA+','PEPC+ ICL+','PEPC+ LPD+','PEPC+ MALout-','PEPC+ SDH+',...
    'PFK+ FBP-','PFK+ FUMA+','PFK+ ICL+','PFK+ LPD-','PFK+ MALout+','PFK+ PEPC+','PFK+ PGL-','PFK+ PYK-','PFK+ SDH+','PFK+ TAL-','PFK+ ZWF-',...
    'SDH+ FUMA+','SDH+ ICL+','SDH+ MALout+',...
    'TAL- FUMA+','TAL- ICL+','TAL- LPD-','TAL- MALout+','TAL- PEPC+','TAL- SDH+',...
    'PYK- FUMA+', 'PYK- ICL+', 'PYK- LPD-', 'PYK- MALout', 'PYK- PEPC+', 'PYK- PGL-', 'PYK- SDH+', 'PYK- TAL-', 'PYK- ZWF-'
    };

idx_pair_all  = [
    8, 14, 15, 16, 17, 19, 20, 23, 24, 11,...
    118, 120, 123, 126, 127, 128, 129, 130, 132, 133, 136, 137,...
    217, 220, 223, 224, 225, 226, 228, 231,...
    31, 34,...
    62, 65, 68, 70,...
    140, 143, 146, 147, 148, 150, 153,...
    77,...
    85, 88, 91, 92, 94,...
    98, 100, 103, 106, 107, 108, 109, 111, 112, 115, 116,...
    177, 180, 183,...
    217, 220, 208, 209, 210, 212,...
    159, 162, 165, 166, 167, 168, 170, 173, 174
    ];
...
barh(1:nn, -v1, 0.6, 'FaceColor',[212, 127, 102] / 255); 

hold on;

% Plot right bars (positive values so they extend right)
barh(1:nn,  v2, 0.6, 'FaceColor',[77, 116, 140]/255);
