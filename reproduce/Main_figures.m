...
result_ranges3=struct();
ACNidx=find(strcmp(enzNames,'ACN'));
for p=1:(length(enzNames))
    nameACN='ACN';
    nameP=enzNames{p};
    rangeACN=enzRanges{ACNidx};
    rangeP=enzRanges{p};
    if strcmp(nameP, 'ACN')
        continue
    else
        pairKey = sprintf('%s__%s', nameACN, nameP);
        
        rangecell=cell(2,1);
        rangecell{1}=rangeACN;
        rangecell{2}=rangeP;
        
        result_ranges3.(pairKey)=rangecell; 
    end   
end

result_ranges=result_ranges1;
fieldsA = fieldnames(result_ranges2);
fieldsB = fieldnames(result_ranges3);
for k = 1:numel(fieldsA)
    result_ranges.(fieldsA{k}) = result_ranges2.(fieldsA{k});
end
for m = 1:numel(fieldsB)
    result_ranges.(fieldsB{m}) = result_ranges3.(fieldsB{m});
end
