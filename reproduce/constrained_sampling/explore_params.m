...
%
% This MATLAB script allows to explore parameter space to conduct analysis
%
% Author: rourouj2@illinois.edu
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function explore_params(batch_index)
    % Validate input
    if nargin < 1
    ...
    %% Basic setting
    rng(42);%can be removed if want different LHS samples every time
    ...
    lhs_struct_array(num_samples) = Vmax;  %create a struct array for later E_lhs 

    %% LHS matrix
    [paramAll_min, paramAll_max] = param_ranges(param_names);
    [P_scaled_values]=lhsdesign_loguniform(num_samples, paramAll_min, paramAll_max);

    save('./P_scaled_values.mat','P_scaled_values');

    fprintf(fid, 'P_scaled_values is loaded at %s with size [%d %d]\n', datestr(now), size(P_scaled_values,1), size(P_scaled_values,2));

    %% regenerate a Vmax LHS matrix
    idx_TKT1 = find(strcmp(param_names, 'TKT1'));
    idx_TKT2 = find(strcmp(param_names, 'TKT2'));  

    Kcat_TKT1=131.2;
    Kcat_TKT2=69.05;

    E_TKT=P_scaled_values(:,idx_TKT1);
    Vmax_TKT1=E_TKT .* Kcat_TKT1;
    Vmax_TKT2=E_TKT .* Kcat_TKT2;
    
    P_scaled=zeros(num_samples, num_params);
    P_scaled(:,1:idx_TKT1-1)=P_scaled_values(:,1:idx_TKT1-1);
    P_scaled(:,idx_TKT1)=Vmax_TKT1;
    P_scaled(:,idx_TKT2)=Vmax_TKT2;
    P_scaled(:,idx_TKT2+1:end)=P_scaled_values(:,idx_TKT1+1:end);
    ...
    parfor i = start_idx:end_idx
        ...
        JRRLHSmatrix=lhs_struct_array(i);%Get the parameter set for this run
        
        % Solve ODE
        [ T, X, FLUXss ] = JRRrunSimulation(JRRLHSmatrix,tspan,x0,options);
        Xss = X(end,:); %steady state concentration
 
        % Check if the last time point equals tmax
        if ~isempty(T) && T(end) == tspan(end)  
            slope=JRRModel_ODE(T(end),X(end,:),JRRLHSmatrix);
            ...
            if all(abs(slope) < 4e-6)
                ...
                Vmax_LHS = cell2mat(struct2cell(JRRLHSmatrix))';
