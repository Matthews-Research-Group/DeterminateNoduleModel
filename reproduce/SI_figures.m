...
%% Figure S19
% extract sets that metabolite concentration of key amino acids are within the range
keyamino = {'ASP','SRN','ALN','GLU','GLN'};

% row index of each key amino acid in newdata.x (same order as metabolite_names)
[found, idx_keyamino] = ismember(keyamino, metabolite_names);
if ~all(found)
    error('Not in metabolite_names: %s', strjoin(keyamino(~found), ', '));
end

% one row per set, one column per key amino acid
x_keyamino = array2table(newdata.x(idx_keyamino, :).', 'VariableNames', keyamino);
x_keyamino.Properties.RowNames = compose('set%d', (1:size(newdata.x,2)).');

% per amino acid, keep the sets whose concentration falls inside its own range
range.ASP = [1.04,  7.77];
range.SRN = [1.55,  3.62];
range.ALN = [0, 15.53]; %[1.55, 15.53];
range.GLU = [1.55, 22.26];
range.GLN = [1.55,  4.40];

% widen every range by tol as a fraction of each bound: [lo*(1-tol), hi*(1+tol)]
tol =  0.30;
inrange = @(v, r) v >= r(1) * (1 - tol) & v <= r(2) * (1 + tol);

x_ASP = x_keyamino(inrange(x_keyamino.ASP, range.ASP), :);
x_SRN = x_keyamino(inrange(x_keyamino.SRN, range.SRN), :);
x_ALN = x_keyamino(inrange(x_keyamino.ALN, range.ALN), :);
x_GLU = x_keyamino(inrange(x_keyamino.GLU, range.GLU), :);
x_GLN = x_keyamino(inrange(x_keyamino.GLN, range.GLN), :);

fprintf('sets in range (of %d): ASP %d, SRN %d, ALN %d, GLU %d, GLN %d\n', ...
        height(x_keyamino), height(x_ASP), height(x_SRN), ...
        height(x_ALN), height(x_GLU), height(x_GLN));

% intersection: sets inside all five ranges at once
keep_all = true(height(x_keyamino), 1);
for k = 1:numel(keyamino)
    keep_all = keep_all & inrange(x_keyamino.(keyamino{k}), range.(keyamino{k}));
end

idx_intersect = find(keep_all);

% all 60 metabolites, one row per set
x_intersect = array2table(newdata.x(:, keep_all).', 'VariableNames', metabolite_names);
x_intersect.Properties.RowNames = x_keyamino.Properties.RowNames(keep_all);

fprintf('sets inside all five ranges: %d -> %s\n', ...
        numel(idx_intersect), mat2str(idx_intersect.'));

% unperturbed baselines of these sets
gcgn_orig   = newdata.gCgN(:); % force a column: gCgN is stored as a row

gcgn_hist = gcgn_orig(idx_intersect);
Nfix_hist = newdata.N_ExportRate(idx_intersect);   % == v(ALTNOut) * 4

figH = figure('Units', 'pixels', 'Position', [200, 200, 1500, 650]);
tiledlayout(figH, 1, 2, 'TileSpacing', 'compact', 'Padding', 'loose');

% A. gCgN
axH1 = nexttile;
hh1 = histogram(axH1, gcgn_hist, 'BinWidth', 0.5, ...
                'FaceColor', [0.85, 0.6, 0.25], 'Normalization', 'count');
hh1.EdgeColor = 'k';  hh1.LineWidth = 2;
xline(axH1, 2, '--k', 'LB=2', 'LabelHorizontalAlignment', 'right', ...
      'FontSize', 30, 'LineWidth', 3);
xline(axH1, 8, '--k', 'UB=8', 'LabelHorizontalAlignment', 'left', ...
      'FontSize', 30, 'LineWidth', 3);
xlim(axH1, [1, 200]);                       % full decades, as in the reference
xticks(axH1, [1 10 100]);
xticklabels(axH1, {'10^{0}', '10^{1}', '10^{2}'});
ylim(axH1, [0, max(hh1.Values) * 1.15]);
xlabel(axH1, '$\mathbf{Nitrogen \; fixation \; efficiency \; (gC g}^{-1}\mathbf{N)}$', ...
       'FontSize', 30, 'Interpreter', 'latex');
ylabel(axH1, '\textbf{Counts}', 'FontSize', 30, 'Interpreter', 'latex');
axH1.FontSize = 30;  axH1.LineWidth = 3;  axH1.XScale = 'log';

% B. N-fixation rate
axH2 = nexttile;
hh2 = histogram(axH2, Nfix_hist, 'BinWidth', 0.25, ...
                'FaceColor', [0.2549, 0.5961, 0.6745], 'Normalization', 'count');
hh2.EdgeColor = 'k';  hh2.LineWidth = 2;
xlim(axH2, [0.1, 12]);                     
xticks(axH2, [0.1 1 10]);                   
xticklabels(axH2, {'10^{-1}', '10^{0}', '10^{1}'});
ylim(axH2, [0, max(hh2.Values) * 1.15]);
xlabel(axH2, '$\textbf{Nitrogen fixation rate (mM s}^{-1}\textbf{)}$', ...
       'FontSize', 30, 'Interpreter', 'latex');
ylabel(axH2, '\textbf{Counts}', 'FontSize', 30, 'Interpreter', 'latex');
axH2.FontSize = 30;  axH2.LineWidth = 3;  axH2.XScale = 'log';

labelsH = {'A.', 'B.'};
for i = 1:2
    axi = [axH1, axH2]; axi = axi(i);
    xl = xlim(axi);  yl = ylim(axi);
    xpos = xl(1) * (xl(2)/xl(1))^0.02;  
    text(axi, xpos, yl(2) - 0.02*diff(yl), labelsH{i}, ...
...
%% Figure S20
T=load('../data/FigureS20data.mat').T;

v1t = T.gcgn_pct_median;    % negative = gCgN drops below baseline -> bar extends left
v2t = T.rate_pct_median;    % positive = N export above baseline   -> bar extends right


med_tol = 0.1;   % percent
keepT = (abs(v1t) > med_tol | abs(v2t) > med_tol) & ~isnan(v1t) & ~isnan(v2t);

v1t     = v1t(keepT);
v2t     = v2t(keepT);
labelsT = T.pair(keepT);
nnT     = numel(v1t);

% sort so the largest gcgn reduction (most negative median) sits on top
[~, ordT] = sort(v1t, 'ascend');
v1t = v1t(ordT);   v2t = v2t(ordT);   labelsT = labelsT(ordT);

figT = figure('Position',[200 200 900 700]);

axT = gca;
axT.Color = [240 240 240]/255;   % light gray background
axT.FontSize = 15;
box(axT, 'on');
axT.LineWidth = 3;
...
hbT1 = barh(1:nnT, v1t, 0.6, 'FaceColor',[212, 127, 102] / 255);
hbT2 = barh(1:nnT, v2t, 0.6, 'FaceColor',[77, 116, 140] / 255);
...
set(gca,'YTick',1:nnT,'YTickLabel',labelsT,'YDir','reverse','FontSize', 23);
xline(0,'k','LineWidth',1.5);

xmaxT = max(abs([v1t; v2t]), [], 'omitnan');
if isempty(xmaxT) || ~isfinite(xmaxT) || xmaxT == 0
    xmaxT = 1;
end
xlim([-1.1*xmaxT, 1.1*xmaxT]);

xlabel('% change','FontSize', 40,'FontWeight','bold');

ylT = ylabel('Pairwise enzyme change','FontSize', 40,'FontWeight','bold');
ylT.Units = 'normalized';
ylT.Position = [-0.03, 0.55, 0];

lgdT = legend([hbT1 hbT2], ...
              {'N-fixation efficiency (gC g^{-1}N)', ...
               'N-fixation rate (mM s^{-1})'}, ...
               'Location', 'southeast', ...
               'FontSize', 25, ...
